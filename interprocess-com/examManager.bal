...
service ExamManagement on httpListener {
    ...
    resource function remarkMngr(http:Caller caller, http:Request request) returns error? {
        ...
        json remarkFullDetails ={"Student Details":studentResponseJSON,"Course and Lecturer info":assignedLecturerDetails};
        
        // Send response to the user
        response.setJsonPayload(untaint remarkFullDetails);
        checkpanic caller->respond(response);
        return;
    }
}